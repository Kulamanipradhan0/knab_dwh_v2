...
where as_of_date=(select max(date_key) from dwh_own.dim_calendar_date where year=(select Extract(year from now())-3) and week_end='Weekday' and holiday='No holiday')
group by Extract(year FROM as_of_date) ,c.bsn_number,c.full_name
order by Extract(year FROM as_of_date) ,c.bsn_number,c.full_name ;'''
